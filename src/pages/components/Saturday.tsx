...
const Saturday: React.FC = (): JSX.Element => {
  ...
  const showModal = (item: number): void => {
    ...
    if (screenWidth <= 1024 && /Android|webOS|iPhone|iPad|Mac|Macintosh|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        ...
        alert({
          cssClass: 'alert-body',
          header: SaturdayAlertText.get(item).title,
          message: SaturdayAlertText.get(item).text,
          buttons: ["fechar"],
          onDidDismiss: () => {},
        });
        ...
        Swal.fire( {
            title: SaturdayAlertText.get(item).title,
            text: SaturdayAlertText.get(item).text,
